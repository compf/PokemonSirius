...
public class PokemonBattleScreen extends HierarchicalObject {
    ...
    public PokemonBattleScreen(int x, int y, int width, int height, Player player, Player enemy) {
        ...
        this.player = player;
        hpBarTextures = new Texture[] { getPixmapTexture(Color.GREEN), getPixmapTexture(Color.YELLOW),
                getPixmapTexture(Color.RED) };
        ...
        playerThread = new Thread((() -> {
            try {
                playerClient = new BattleClient(battleRule, player.getName(), player.getTeam(),
                        SharedPipe.getOrCreatePipe(PLAYER_SERVER_PORT), playerInput);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }));
        ...
        botThread = new Thread((() -> {
            try {
                botClient = new BattleClient(battleRule, "Bot", botPlayer.getTeam(),
    ...
    public PokemonBattleScreen(int x, int y, int width, int height, Player player, Pokemon enemy) {
        this(x, y, width, height, player, new Player((short) 1, "Wild " + enemy.getName(),
                new Pokemon[] { enemy, null, null, null, null, null }));
    ...
    public class BattleControl extends BaseMenu<String> implements IOInterface {
        ...
        public synchronized PlayerInput requestPlayerInput(short pkmnIndex, BattleState state) {
            try {
                ...
                return new PlayerInput.AttackInput(pkmnIndex, moveIndex, targetPlayer, targetPokemon,
                        player.getPlayerId());
