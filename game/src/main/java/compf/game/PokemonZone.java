...
public class PokemonZone extends RepeatableTexture {
    ...
    public void onKeyPress(int keyCode, boolean newPress) {
        if(keyCode==Settings.KEY_UP || keyCode==Settings.KEY_DOWN || keyCode==Settings.KEY_LEFT|| keyCode==Settings.KEY_RIGHT){
    ...
    public PokemonZone(int x, int y, int width, int height, Texture texture, List<PokemonZoneEntry> pkmnZoneEntries,int probEncounter) {
    ...
    public Pokemon getRandomPokemon(){
        ...
        for(var zone:pokemonZoneEntries){
            sum+=zone.Weight;
            if(sum>randVal)return zone.createPokemon();
        }
