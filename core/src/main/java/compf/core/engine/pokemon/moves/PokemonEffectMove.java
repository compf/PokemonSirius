		...
		if (MyObject.checkPerc(_perc)) {
			...
			try {
				if (_on_attacker != null) {
					var att = (Object) _on_attacker.getDeclaredConstructor(Pokemon.class)
							.newInstance(item.getAttacker());
					attEffect = (PokemonBattleEffect) att;
				}
				if (_on_defender != null) {
					var defE = (Object) _on_defender.getDeclaredConstructor(Pokemon.class)
							.newInstance(item.getDefender());
