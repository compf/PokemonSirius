...
public class HumanPlayer {
	 ...
	 * do {
	 * index = Prompt.prompt("Please choose a pokemon", (p, i) -> p.toString(),
	 * _team);
	 * 
	 * nextPokemon = _team[index] != null && _team[index].getCurrHP() > 0 ?
	 * _team[index] : null;
	 * if(index<=original) {
	 * nextPokemon=null;
	 * }
	 ...
	 * 
	 * @Override
	 * public short chooseTarget(MultiplePokemonBattle battle,boolean isEnemy,byte
	 * pkmnId,Move mv) {
	 * byte b1,b2;
	 * while(true) {
	 * b1=battle.promptPlayer(this,isEnemy);
	 * b2=(byte)Prompt.
	 * prompt("Select pokemon from player or other value to choose another player",(
	 * pk,i)->pk.toString(),(i,pk)->i<pk.getPlayer().getPokemonCount(),battle.
	 * getPlayers().get(b1).getTeam());
	 * if(b2!=-1)break;
	 * }
	 * 
